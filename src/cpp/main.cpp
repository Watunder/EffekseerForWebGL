...
extern "C"
{
	...
	EfkWebViewer::Context* EXPORT Init(int instanceMaxCount, int squareMaxCount, bool isExtentionsEnabled, bool isPremultipliedAlphaEnabled)
	{
	...
	void EXPORT Terminate(EfkWebViewer::Context* context)
	{
		context->Terminate();
		delete context;
	}

	void EXPORT Update(EfkWebViewer::Context* context, float deltaFrames)
	{
	...
	void EXPORT BeginUpdate(EfkWebViewer::Context* context)
	{
		context->manager->BeginUpdate();
		context->isFirstUpdate_ = true;
	}

	void EXPORT EndUpdate(EfkWebViewer::Context* context)
	{
		context->manager->EndUpdate();
		context->renderer->SetTime(context->time_);
	}

	void EXPORT UpdateHandle(EfkWebViewer::Context* context, int handle, float deltaFrame)
	{
	...
	void EXPORT Draw(EfkWebViewer::Context* context) { context->Draw(); }

	void EXPORT BeginDraw(EfkWebViewer::Context* context)
	{
	...
	void EXPORT EndDraw(EfkWebViewer::Context* context) { context->renderer->EndRendering(); }

	void EXPORT DrawHandle(EfkWebViewer::Context* context, int handle) { context->manager->DrawHandle(handle); }

	void EXPORT SetProjectionMatrix(EfkWebViewer::Context* context, const float* matrixElements)
	{
		ArrayToMatrix44(matrixElements, context->projectionMatrix);
	}

	void EXPORT SetProjectionPerspective(EfkWebViewer::Context* context, float fov, float aspect, float near, float far)
	{
		context->projectionMatrix.PerspectiveFovRH_OpenGL(fov * 3.1415926f / 180.0f, aspect, near, far);
	}

	void EXPORT SetProjectionOrthographic(EfkWebViewer::Context* context, float width, float height, float near, float far)
	{
		context->projectionMatrix.OrthographicRH(width, height, near, far);
	}

	void EXPORT SetCameraMatrix(EfkWebViewer::Context* context, const float* matrixElements)
	{
		ArrayToMatrix44(matrixElements, context->cameraMatrix);
	}

	void EXPORT SetCameraLookAt(EfkWebViewer::Context* context,
										 float eyeX,
										 float eyeY,
										 float eyeZ,
										 float atX,
	...
	void* EXPORT LoadEffect(EfkWebViewer::Context* context, void* data, int32_t size, float magnification)
	{
		auto effect = Effect::Create(context->manager, data, size, magnification);
		return effect.Pin();
	}

	void EXPORT ReleaseEffect(EfkWebViewer::Context* context, void* effect) { Effekseer::RefPtr<Effect>::Unpin(effect); }

	void EXPORT ReloadResources(EfkWebViewer::Context* context, Effect* effect, void* data, int32_t size)
	{
	...
	void EXPORT StopAllEffects(EfkWebViewer::Context* context) { context->manager->StopAllEffects(); }

	int EXPORT PlayEffect(EfkWebViewer::Context* context, void* effect, float x, float y, float z)
	{
		auto effectRef = Effekseer::RefPtr<Effect>::FromPinned(effect);
		return context->manager->Play(effectRef, x, y, z);
	}

	void EXPORT StopEffect(EfkWebViewer::Context* context, int handle) { context->manager->StopEffect(handle); }

	void EXPORT StopRoot(EfkWebViewer::Context* context, int handle) { context->manager->StopRoot(handle); }

	int EXPORT Exists(EfkWebViewer::Context* context, int handle) { return context->manager->Exists(handle) ? 1 : 0; }

	void EXPORT SetFrame(EfkWebViewer::Context* context, int handle, float frame)
	{
		context->manager->UpdateHandleToMoveToFrame(handle, frame);
	}

	void EXPORT SetLocation(EfkWebViewer::Context* context, int handle, float x, float y, float z)
	{
		context->manager->SetLocation(handle, x, y, z);
	}

	void EXPORT SetRotation(EfkWebViewer::Context* context, int handle, float x, float y, float z)
	{
		context->manager->SetRotation(handle, x, y, z);
	}

	void EXPORT SetScale(EfkWebViewer::Context* context, int handle, float x, float y, float z)
	{
		context->manager->SetScale(handle, x, y, z);
	}

	void EXPORT SetMatrix(EfkWebViewer::Context* context, int handle, const float* matrixElements)
	{
		Matrix43 matrix43;
	...
	float EXPORT GetDynamicInput(EfkWebViewer::Context* context, int handle, int32_t index)
	{
		return context->manager->GetDynamicInput(handle, index);
	}

	void EXPORT SetDynamicInput(EfkWebViewer::Context* context, int handle, int32_t index, float value)
	{
		context->manager->SetDynamicInput(handle, index, value);
	}

	void EXPORT SendTrigger(EfkWebViewer::Context* context, int handle, int32_t index)
	{
		context->manager->SendTrigger(handle, index);
	}

	void EXPORT SetAllColor(EfkWebViewer::Context* context, int handle, float r, float g, float b, float a)
	{
		Color color = Color(r, g, b, a);
		context->manager->SetAllColor(handle, color);
	}

	void EXPORT SetTargetLocation(EfkWebViewer::Context* context, int handle, float x, float y, float z)
	{
		context->manager->SetTargetLocation(handle, x, y, z);
	}

	void EXPORT SetPaused(EfkWebViewer::Context* context, int handle, int paused)
	{
		context->manager->SetPaused(handle, paused != 0);
	}

	void EXPORT SetShown(EfkWebViewer::Context* context, int handle, int shown) { context->manager->SetShown(handle, shown != 0); }

	void EXPORT SetSpeed(EfkWebViewer::Context* context, int handle, float speed) { context->manager->SetSpeed(handle, speed); }

	void EXPORT SetRandomSeed(EfkWebViewer::Context* context, int handle, float seed) { context->manager->SetRandomSeed(handle, seed); }

	int32_t EXPORT GetRestInstancesCount(EfkWebViewer::Context* context) { return context->manager->GetRestInstancesCount(); }

	int EXPORT GetUpdateTime(EfkWebViewer::Context* context) { return context->manager->GetUpdateTime(); }

	int EXPORT GetDrawTime(EfkWebViewer::Context* context) { return context->manager->GetDrawTime(); }

	int EXPORT IsVertexArrayObjectSupported(EfkWebViewer::Context* context)
	{
	...
	void EXPORT SetRestorationOfStatesFlag(EfkWebViewer::Context* context, int flag)
	{
		if (context->renderer == nullptr)
			return;
	...
	void EXPORT CaptureBackground(EfkWebViewer::Context* context, int x, int y, int width, int height)
	{
		if (context == nullptr)
			return;
	...
	void EXPORT ResetBackground(EfkWebViewer::Context* context)
	{
		if (context == nullptr)
			return;
	...
	void EXPORT SetLogEnabled(int flag) { isEffekseerLogEnabled = flag > 0; }
}
